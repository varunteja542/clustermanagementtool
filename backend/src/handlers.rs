...
pub async fn get_clusters() -> Json<serde_json::Value> {
    // Return a list of clusters (placeholder data)
    Json(json!([
        { "id": 1, "name": "Cluster A" },
        { "id": 2, "name": "Cluster B" }
    ]))
...
pub async fn get_nodes(Path(_id): Path<u32>) -> Json<serde_json::Value> {
    // Return a list of nodes in a cluster (placeholder data)
    Json(json!([
...
pub async fn get_node_actions(Path(id): Path<String>) -> Json<serde_json::Value> {
    // Return a list of previous actions for a node (placeholder data)
    Json(json!([
